...
module Cardano.Plutus.OnChain.Bridge.NonceInteger where
...
type EvmNonce = Integer

{-# INLINABLE nextNonce #-}
nextNonce :: EvmNonce -> Integer -> EvmNonce
nextNonce = (+)
...
mkNonceValidator owner datum@NonceDatum{..} redeemer ctx@ScriptContext{scriptContextTxInfo=txIn} =
    ...
    where
        ...
        -- TODO: maybe refactor so that current holder can delete nonce
        dissolveOk = valuePaidTo txIn owner == valueProduced txIn
        ...
        nonceUpdateOk NonceRedeemer{..} = if redeemerMatchesDatum then hasOutDatum else False
            where
                redeemerMatchesDatum = newEvmNonce == nextNonce evmNonce evmTxs
